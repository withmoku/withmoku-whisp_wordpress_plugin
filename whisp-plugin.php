...
register_activation_hook( __FILE__, 'whisp_activate' );
...
register_deactivation_hook( __FILE__, 'whisp_remove_database' );
...
add_action('admin_menu', 'whisp_add_pages');
...
<form id="myForm" name="myform" action="' . esc_attr( admin_url('admin-post.php') ).'" method="POST">
  <input type="hidden" name="action" value="save_my_custom_form" />
    <table>
      <tr>
        <td style="padding-bottom: 20px;">Product Identifier</td>
        <td style="padding-left: 10px;"><input name="whisp_pid" type="text" /><br /><br /></td>
      </tr>
      <tr>
        <td style="padding-bottom: 20px;">WHISP Type</td>
        <td style="padding-left: 10px;">
         <select id="name" name="whisp_type" size="1">
      ...
      <tr>
        ...
        <td style="padding-left: 10px;"><input type="text" name="whisp_source"/><br /><br /></td>
      </tr>
      <tr>
        <td style="padding-bottom: 20px;">CAMPAIGN</td>
        <td style="padding-left: 10px;"><input type="text" name="whisp_campaign" /><br /><br /></td>
      </tr>
      <tr>
        <td style="padding-bottom: 20px;">OTHER</td>
        <td style="padding-left: 10px;"><input type="text" name="whisp_other" /><br /><br /></td>
      </tr>
      <tr>
        <td></td>
  ...
  foreach ( $result as $print ) {
    ...
    $code = $print->code;
    echo "<tr>";
    echo "<td>".esc_attr(esc_attr($pid))."</td>";
    echo "<td>".esc_attr(esc_attr($type))."</td>";
    echo "<td>".esc_attr(esc_attr($source))."</td>";
    echo "<td>".esc_attr(esc_attr($campaign))."</td>";
    echo "<td>".esc_attr(esc_attr($other))."</td>";
    echo "<td>".esc_attr(esc_attr($code))."</td>";
    echo "<td><button 
    onclick='whisp_delete(".esc_attr(esc_attr($id)).")' class='btn btn-danger btn-sm'>
    Delete
  </button></td>";
    echo "</tr>";
   }
 ...
 function whisp_save_form() {
    global $wpdb;
    $pid = $_POST['whisp_pid'];
    $whisptype = $_POST['whisp_type'];
    $source = $_POST['whisp_source'];
    $campaign = $_POST['whisp_campaign'];
    $other = $_POST['whisp_other'];
    ...
    wp_redirect( site_url('/wp-admin/admin.php?page=whisp_form') ); // <-- here goes address of site that user should be redirected after submitting that form
    die;
}
add_action( 'admin_post_nopriv_save_my_custom_form', 'whisp_save_form' );
add_action( 'admin_post_save_my_custom_form', 'whisp_save_form' );
...
add_shortcode('WHISP', 'whisp_shortcode_function');
function whisp_shortcode_function($atts) {
 ...
 $product_id = $a['button_type'];
  switch( $product_id ){
    case 'wheel': 
        $output =  '<div id="button_wtf"><img width="150px" class="taptext-wheel btn-tap-text" src="'.plugin_dir_url( __FILE__ ).'assets/spinthewheel.png'.'"  /></div>';
        echo wp_get_script_tag( array(
            'type' => 'text/javascript',
            'id' => 'taptext-lib',
            'data-chatbtn' => 'true',
            'data-productidentifer' => esc_attr($a['product_identifier']),
            'src' => 'https://hub.taptext.com/scripts/taptext_lib.js'
        ) );
        break;

    case 'text': 
        $output =  '<div id="button_wtf">
    <a href="javascript:" class="pink-btn" data-itemurl="" data-utm_source="" data-utm_medium="" data-utm_campaign="" data-utm_term="" data-utm_content="" data-productidentifer="' . esc_attr($a['product_identifier']) . '" onclick="onTapTextClick(this)" style="float: left; cursor:pointer"><img src="'.plugin_dir_url( __FILE__ ).'assets/taptext.png'.'" width="150px" style="float: left;" /></a></div>';
        echo wp_get_script_tag( array(
            'type' => 'text/javascript',
            'id' => 'taptext-lib',
            'data-chatbtn' => 'true',
